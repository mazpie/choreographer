...
def make(task_id, obs_type, seed, img_size=84, exorl=False):
  ...
  # use_site matches EXORL
  if obs_type == 'states' and not exorl:
    global _TIME_LIMIT
    _TIME_LIMIT = 10.04
    # Note: Adding this fixes the problem of having 249 steps with action repeat = 1
  task = _reach(task_id, obs_settings=obs_settings, use_site=True if exorl else False)
